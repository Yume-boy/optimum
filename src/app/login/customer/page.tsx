...
const LoginForm = () => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
       const response = await login({
        ...
        sessionStorage.setItem('user', response?.user.fullname)
        sessionStorage.setItem('email', response?.user.email)
        sessionStorage.setItem('token', response.token)
        sessionStorage.setItem('role', response.role)
        sessionStorage.setItem('phone', response?.user.phone)
        sessionStorage.setItem('home_address', response?.user.home_address)
        sessionStorage.setItem('user_id', response?.user.id)
        sessionStorage.setItem('work_address', response?.user.work_address)
        sessionStorage.setItem('other_address', response?.user.other_address)
        router.push('/customer/dashboard')
    } catch (err) {
      console.log(err); // Using alert for demo
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#ececec] flex items-center justify-center p-4 font-sans">
      ...
      <div className='flex gap-2 items-center '><ArrowLeft className='absolute top-5 left-2' onClick={()=> router.back()}/></div>
