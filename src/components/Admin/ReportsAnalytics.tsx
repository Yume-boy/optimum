...
// NOTE: Removed `import { useApp } from '../../contexts/AppContext';` 
// and the destructuring `{ orders, drivers } = useApp()` to only rely on dummy data.

const ReportsAnalytics: React.FC = () => {
  // Removed const { orders, drivers } = useApp();
...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 w-full md:w-auto"> {/* Adjusted for mobile stacking */}
          <select
            value={dateRange}
            onChange={(e) => setDateRange(e.target.value)}
            className="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
          >
...
          </select>
          <div className="flex space-x-2 w-full sm:w-auto"> {/* Ensure buttons also fill width on mobile */}
            <button
              onClick={() => exportReport('csv')}
              className="flex items-center justify-center space-x-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 w-1/2 sm:w-auto"
            >
...
              className="flex items-center justify-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 w-1/2 sm:w-auto"
            >
              <Download className="h-4 w-4" />
              <span>PDF</span>
            </button>
...
        <div className="border-b border-gray-200 overflow-x-auto"> {/* Added overflow for small screens */}
          <nav className="flex space-x-6 sm:space-x-8 px-4 sm:px-6 min-w-max"> {/* min-w-max prevents wrap on small screens */}
            {[
...
                className={`py-3 sm:py-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap transition duration-150 ${
                  reportType === tab.id
...
        <div className="p-4 sm:p-6"> {/* Responsive padding for content area */}
          {reportType === 'overview' && (
            <div className="space-y-6">
              {/* KPI Cards - Adjusted to md:grid-cols-2 lg:grid-cols-4 for better tablet layout */}
...
              {/* Charts Row - Grid columns adjust from 1 to 2 */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Daily Order Status</h3>
                  <ResponsiveContainer width="100%" aspect={4/3} maxHeight={300}> {/* Added aspect and maxHeight for better control on small screens */}
                    <BarChart data={dailyOrders} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
...
                  <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-4"> {/* Stack chart and legend on small screens */}
                    <div className="w-full sm:w-1/2">
                      <ResponsiveContainer width="100%" aspect={1/1} maxHeight={300}>
...
                    </div>
                    {/* Legend */}
                    <div className="mt-4 sm:mt-0 grid grid-cols-2 gap-2 w-full sm:w-1/2">
                      {categoryData.map((entry) => (
                        <div key={entry.name} className="flex items-center space-x-2">
                          <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: entry.color }} />
                          <span className="text-sm text-gray-600 whitespace-nowrap">{entry.name} ({entry.value}%)</span>
                        </div>
                      ))}
                    </div>
                  </div>
...
                <ResponsiveContainer width="100%" aspect={4/3} maxHeight={300}>
                  <AreaChart data={hourlyActivity} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" />
...
                <ResponsiveContainer width="100%" aspect={4/3} maxHeight={400}>
...
              {/* Financial Cards - Adjusted to md:grid-cols-2 lg:grid-cols-3 */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                  <h4 className="font-semibold text-gray-900 mb-4">Revenue Breakdown</h4>
                  <div className="space-y-3 text-sm"> {/* Added text-sm for better fit on mobile */}
...
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 md:col-span-2 lg:col-span-1"> {/* Take full width on mobile/tablet if only two are present on md */}
...
                <ResponsiveContainer width="100%" aspect={4/3} maxHeight={400}>
                  <BarChart data={driverPerformance} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" />
...
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Rank</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Driver</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Deliveries</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Rating</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Earnings</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Efficiency</th>
                      </tr>
                    </thead>
...
                                ></div>
                              </div>
                              <span className="text-sm text-gray-600 flex-shrink-0">{Math.round((driver.deliveries / 50) * 100)}%</span>
                            </div>
                          </td>
                        </tr>
                      ))}
